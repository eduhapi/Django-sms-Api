...
def external_send_sms(request):
    if request.method == 'POST':
        try:
            ...
            #tenant_app_key = data.get('tenant_app_key')
            phone_number = data.get('phone')
            message = data.get('message')
            
            # Validate tenant app key
           # if tenant_app_key != '8e252ddc9a1a4e7bbca5a986906c584ec77eedf93bf335dffbebd60b862af3a4':
               # return JsonResponse({'status': 'error', 'message': 'Invalid tenant app key'}, status=403)
